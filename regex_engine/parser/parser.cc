...
shared_ptr<RE> Parser::parse_re() { return make_shared<RE>(parse_re_seq()); }

shared_ptr<ASTNode>
Parser::parse_re_seq(bool capturing, const string &group_name, int group_id) {
  bool match_start = false;
  bool match_end = false;

  // Debug: Print the current token at the start of the function
  // std::cout << "parse_re_seq: Current token = " << (curr_tkn ?
  // curr_tkn->char_ : "nullptr") << std::endl;

  // Handle start anchor (^)
  if (dc<Start *>(curr_tkn) || dc<Circumflex *>(curr_tkn)) {
    // cout << "parse_re_seq: Found start anchor (^), setting match_start =
    // true" << std::endl;
    next_tkn();
    match_start = true;
  }
  ...
  if (dc<EndToken *>(curr_tkn)) {
    // cout << "parse_re_seq: Found end anchor ($), setting match_end = true" <<
    // std::endl;
    next_tkn();
    match_end = true;
  }
  ...
  if (match_start) {
    ...
    dynamic_pointer_cast<GroupNode>(node)->children.push_front(
        make_shared<StartElement>());
  }

  // Add EndElement if match_end is true
  if (match_end) {
    // cout << "parse_re_seq: Adding EndElement to the group" << std::endl;
    dynamic_pointer_cast<GroupNode>(node)->children.push_back(
        make_shared<EndElement>());
  }

  // Handle alternation (|)
  if (dc<OrToken *>(curr_tkn)) {
    // cout << "parse_re_seq: Found alternation (|), creating OrNode" <<
    // std::endl;
    next_tkn();
    node = make_shared<OrNode>(
        node, parse_re_seq(true, group_name,
                           dynamic_pointer_cast<GroupNode>(node)->group_id));
  }

  return node;
}

shared_ptr<GroupNode>
Parser::parse_group(bool capturing, const string &group_name, int group_id) {
  if (group_id == -1) {
  ...
  while (curr_tkn && !dynamic_cast<OrToken *>(curr_tkn) &&
    ...
    // cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") <<
    // endl;
    ...
    if (dynamic_cast<Quantifier *>(curr_tkn)) {
      ...
      } else if (auto range_element =
                     dynamic_pointer_cast<RangeElement>(new_el)) {
        if (dynamic_cast<ZeroOrOne *>(curr_tkn)) {
          // cout << "ZeroOrOne quantifier." << endl;
          range_element->min = 0;
          range_element->max = 1;
      ...
      // cout << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr")
      // << endl; cout << "LeftCurlyBrace found, parsing curly..." << endl; cout
      // << "Current token: " << (curr_tkn ? curr_tkn->char_ : "nullptr") <<
      // endl;
      parse_curly(new_el);
    }
  ...
  try {
    ...
    if (dynamic_cast<RightCurlyBrace *>(curr_tkn)) {
      ...
      if (element) {
        ...
        range_element->min = stoi(val_1);
        range_element->max =
            numeric_limits<int>::max(); // Use max() for infinity
      } else if (gn) {
        gn->min = stoi(val_1);
    ...
    if (val_2.empty())
      val_2 = to_string(numeric_limits<int>::infinity());
    // cout << "val_2 is " << val_2 << endl;
    if (!dynamic_cast<RightCurlyBrace *>(curr_tkn)) {
...
shared_ptr<RangeElement> Parser::parse_inner_el() {
  ...
  while (curr_tkn) {
    if (dc<RightBracket *>(curr_tkn))
      break;

    if (dc<SpaceToken *>(curr_tkn)) {
      match_str += dc<SpaceToken *>(curr_tkn)->char_;
      // cout << "This is the space token char in parse_inner_el() " <<
      // dc<SpaceToken*>(curr_tkn)->char_ << endl;
      next_tkn();
      continue;
    }
    ...
    if (peek_tkn && dc<Dash *>(peek_tkn)) {
      ...
      next_tkn();
      // cout << "Yeah consumed 2 tokens here, now on: " << (curr_tkn ?
      // curr_tkn->char_ : "nullptr") << endl;

      Token *next_peek_tkn = next_tkn(true);

      if (dc<RightBracket *>(next_peek_tkn) ||
          dc<SpaceToken *>(next_peek_tkn)) {
        match_str += prev_char;
...
shared_ptr<ASTNode> Parser::parse_el() {
  ...
  // cout << "parse_el: Current token = " << (curr_tkn ? curr_tkn->char_ :
  // "nullptr") << std::endl;
  ...
  if (dynamic_cast<ElementToken *>(curr_tkn)) {
    ...
    // cout << "parse_el: Found Wildcard, creating WildcardElement node" <<
    // std::endl;
    return make_shared<WildcardElement>();
  } else if (dynamic_cast<SpaceToken *>(curr_tkn)) {
    // Debug: Handling SpaceToken
    // cout << "parse_el: Found SpaceToken, creating SpaceElement node" <<
    // std::endl;
    ...
    // cout << "parse_el: Found LeftParenthesis, starting group parsing" <<
    // std::endl;
    ...
    if (dynamic_cast<QuestionMark *>(curr_tkn)) {
      // cout << "parse_el: Found QuestionMark, checking for non-capturing or
      // named group" << std::endl;
      next_tkn(); // Move past the '?'

      if (dynamic_cast<ElementToken *>(curr_tkn) &&
          dynamic_cast<ElementToken *>(curr_tkn)->char_ == ":") {
        // Debug: Non-capturing group
        // cout << "parse_el: Found ':', marking group as non-capturing" <<
        // std::endl;
        capturing = false;
        next_tkn(); // Move past the ':'
      } else if (dynamic_cast<ElementToken *>(curr_tkn) &&
                 dynamic_cast<ElementToken *>(curr_tkn)->char_ == "<") {
        // Debug: Named group
    ...
    if (dynamic_cast<RightParenthesis *>(curr_tkn)) {
      // cout << "parse_el: Found RightParenthesis, group parsing complete" <<
      // std::endl;
      return res;
    } else {
    ...
    // cout << "parse_el: Unhandled token = " << (curr_tkn ? curr_tkn->char_ :
    // "nullptr") << std::endl;
...
string Parser::parse_group_name() {
  ...
  while (curr_tkn && dynamic_cast<ElementToken *>(curr_tkn) &&
         dynamic_cast<ElementToken *>(curr_tkn)->char_ != ">") {
  ...
  if (group_name.empty())
    throw runtime_error("Unexpected empty named group name.");

  // consume the > token
  if (curr_tkn && dynamic_cast<ElementToken *>(curr_tkn) &&
      dynamic_cast<ElementToken *>(curr_tkn)->char_ == ">") {
    next_tkn();
  } else {
