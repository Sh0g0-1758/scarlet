...
private:
  ...
  register_type index;
  /*
   * For Memory operands, this is used for register based indexing
   * (so the value is present at some offset from the register)
   *
   * For PseudoMem operands, this is used as the offset from the array
   * (so store the value at some offset from the array)
   *
   * For Indexed operands, this is used as the scale
   * (so final value will be base + index * scale)
   */
  long offset;
  cond_code cond;
...
public:
  ...
  void add_init(constant::Constant init) { this->init.emplace_back(init); }
