...
void parser::parse_simple_declarator(
  ...
  if (tokens[0].get_token() == token::TOKEN::IDENTIFIER) {
    ...
    // simple_declarator->add_derived_type(ast::DeclarationType::VARIABLE);
  } else {
...
void parser::parse_direct_declarator(
  ...
  if (tokens[0].get_token() == token::TOKEN::OPEN_PARANTHESES) {
    ...
    // direct_declarator->add_derived_type(ast::DeclarationType::VARIABLE);
    direct_declarator->set_simple_declarator(std::move(simple_declarator));
  }
}
...
bool parser::isFuncDecl(std::shared_ptr<ast::AST_declarator_Node> decl) {
  ...
  return false;
  // if(decl->get_derived_type().size() != 0 && decl->get_derived_type()[0] ==
  // ast::DeclarationType::FUNCTION) {
  //   return true;
  // }
  // return false;
...
void parser::parse_declaration(std::vector<token::Token> &tokens,
  ...
  std::cout << declarator->get_derived_type().size() << std::endl;
  for (auto i : declarator->get_derived_type()) {
    if (i == ast::DeclarationType::POINTER) {
      std::cout << "POINTER" << " ";
    }
    if (i == ast::DeclarationType::FUNCTION) {
      std::cout << "FUNCTION" << " ";
    }
    if (i == ast::DeclarationType::VARIABLE) {
      std::cout << "VARIABLE" << " ";
    }
  }
  std::cout << " " << std::endl;
