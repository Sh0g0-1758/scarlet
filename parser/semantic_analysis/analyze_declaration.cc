...
void parser::analyze_declaration(
    ...
    int indx) {
  std::string var_name;
  if (declaration->get_declarator()->get_identifier() != nullptr) {
    var_name = declaration->get_declarator()->get_identifier()->get_value();
  } else {
    success = false;
    error_messages.emplace_back("Expected identifier but got");
  }
  ...
  if (funcDecl->get_block() == nullptr) {
    ...
    for (auto param : funcDecl->get_params()) {
      ...
      globalSymbolTable[temp_name] = proxy_symbol_table[{
          param->get_declarator()->get_identifier()->get_value(), 1}];
      param->get_declarator()->set_identifier(temp_name);
    }
...
void parser::analyze_local_variable_declaration(
  ...
  if (varDecl->get_specifier() == ast::SpecifierType::EXTERN) {
    ...
    varDecl->get_declarator()->set_identifier(temp_name);
