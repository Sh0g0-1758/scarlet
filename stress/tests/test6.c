...
compute_monster_1:
    if ((((x = y << 4) + (z = i >> 3)) * ((j = k * 13) % 29)) != ((l = m + n) * (o = p - 100))) {
        if ((((a = b ^ c) & (d = e | f)) + ((x = y * z) % (i = j - k))) > ((l = m << 3) + (n = o >> 2))) {
...
compute_beast_2:
    if ((((k = l * m) << 4) + ((n = o * p) >> 3)) > ((a = b + c) * (d = e - f))) {
        x = ((y = z * i) + ((j = k - l) * (m = n / 2)));
        if ((((o = p << 3) & (a = b >> 2)) * ((c = d * e) % (f = x - y))) < ((z = i + j) ^ (k = l * m)))
            goto compute_havoc_4;
        goto compute_doom_6;
    }
    goto megaloop;
...
compute_chaos_3:
    if ((((n = o ^ p) | (a = b & c)) + ((d = e * f) % (x = y + z))) > ((i = j << 4) * (k = l >> 3))) {
...
compute_havoc_4:
    ...
    if ((((x = y << 6) & (z = i >> 4)) * ((j = k * 17) % 31)) > ((l = m + n) ^ (o = p * a))) {
        if ((((b = c * d) + (e = f << 3)) * ((x = y - z) % (i = j + k))) < ((l = m * n) + (o = p >> 2)))
            goto compute_beast_2;
        goto compute_monster_1;
    }
    goto compute_chaos_3;
...
compute_nightmare_5:
    if ((((l = m * n) << 5) + ((o = p * a) >> 3)) != ((b = c + d) * (e = f - x))) {
        y = ((z = i * j) + ((k = l - m) * (n = o / 2)));
        if ((((p = a << 4) & (b = c >> 3)) * ((d = e * f) % (x = y - z))) > ((i = j + k) ^ (l = m * n)))
            goto compute_doom_6;
        goto compute_havoc_4;
    }
    goto megaloop;
...
compute_doom_6:
    if ((((o = p ^ a) | (b = c & d)) + ((e = f * x) % (y = z + i))) < ((j = k << 5) * (l = m >> 4))) {
        n = ((o = p * 29) % ((a = b + 31) * (c = d - 41)));
        if ((((e = f * x) << 6) + ((y = z * i) >> 5)) > ((j = k + l) * (m = n - o))) {
            if ((((p = a * b) + (c = d << 4)) * ((e = f - x) % (y = z + i))) > ((j = k * l) + (m = n >> 3)))
                goto compute_nightmare_5;
            goto compute_chaos_3;
        }
        goto compute_monster_1;
